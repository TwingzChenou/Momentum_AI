...
async def fetch_income_statement_for_ticker(session, semaphore, ticker):
    """
    Fetches the quarterly income statements for a single ticker.
    Limits concurrency using a semaphore and implements exponential backoff for 429 Rate Limits.
    """
    url = f"{BASE_URL}/income-statement?symbol={ticker}&limit=1000&period=quarterly&apikey={FMP_API_KEY}"
...
async def fetch_all_income_statements(tickers):
    """
    Manages the asynchronous fetching of income statements for the entire list of tickers.
    """
    ...
    async with aiohttp.ClientSession() as session:
        for ticker in tickers:
            task = fetch_income_statement_for_ticker(session, semaphore, ticker)
            tasks.append(task)
...
def load_unique_tickers_from_history(spark):
    ...
    try:
        ...
        # We only need unique tickers to fetch income statements
...
def save_income_statements_to_lake(spark, pandas_df):
    """
    Saves the fetched income statements DataFrame to Delta Lake.
    """
    logger.info(f"💾 Saving {pandas_df.shape[0]} income statement records to {Paths.SP500_INCOME_STATEMENT}...")
    ...
    try:
        ...
        sdf.write.format("delta") \
            ...
            .save(Paths.SP500_INCOME_STATEMENT)
            
        logger.success("✅ Success! Income Statements saved.")
        
    except Exception as e:
        logger.error(f"❌ Error saving income statements to Lake: {e}")

def main():
    setup_logging()
    logger.info("🚀 Starting Job: List SP500 Income Statements Ingestion")

    spark = None

    try:
        spark = create_spark_session(app_name="List_SP500_Income_Statements")
        
        cache_file = 'local_income_statement_cache.parquet'
        if os.path.exists(cache_file):
            logger.info(f"♻️ Found local cache {cache_file}. Loading from cache...")
            df_is = pd.read_parquet(cache_file)
        else:
            ...
            # 2. Asynchronously fetch income statements from FMP API
            logger.info("⏱️ Fetching income statements... This may take a few minutes.")
            raw_data = asyncio.run(fetch_all_income_statements(tickers))
            ...
            df_is = pd.DataFrame(raw_data)
            logger.info(f"📊 Processed a total of {df_is.shape[0]} income statement rows.")
            
            # Drop duplicates if any overlaps occurred
            if 'symbol' in df_is.columns and 'date' in df_is.columns:
                df_is = df_is.drop_duplicates(subset=['symbol', 'date'])
            ...
            for str_col in df_is.select_dtypes(include=['object']).columns:
                df_is[str_col] = df_is[str_col].astype(str)
                
            df_is.to_parquet(cache_file, index=False)
        
        # 4. Save to Bronze
        save_income_statements_to_lake(spark, df_is)
