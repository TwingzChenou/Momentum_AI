...
async def fetch_key_metrics_for_ticker(session, semaphore, ticker):
    """
    Fetches the quarterly key metrics for a single ticker.
    Limits concurrency using a semaphore and implements exponential backoff for 429 Rate Limits.
    """
    url = f"{BASE_URL}/key-metrics?symbol={ticker}&limit=1000&period=quarterly&apikey={FMP_API_KEY}"
...
async def fetch_all_ratios(tickers):
    ...
    async with aiohttp.ClientSession() as session:
        for ticker in tickers:
            task = fetch_key_metrics_for_ticker(session, semaphore, ticker)
            tasks.append(task)
...
def save_ratios_to_lake(spark, pandas_df):
    ...
    """
    logger.info(f"💾 Saving {pandas_df.shape[0]} key metrics records to {Paths.SP500_KEY_METRICS}...")
    ...
    try:
        ...
        sdf.write.format("delta") \
            ...
            .save(Paths.SP500_KEY_METRICS)
            
        logger.success("✅ Success! Key metrics saved.")
        
    except Exception as e:
        logger.error(f"❌ Error saving key metrics to Lake: {e}")

def main():
    setup_logging()
    logger.info("🚀 Starting Job: List SP500 Key Metrics Ingestion")

    spark = None

    try:
        spark = create_spark_session(app_name="List_SP500_Key_Metrics")
        
        cache_file = 'local_key_metrics_cache.parquet'
        if os.path.exists(cache_file):
            logger.info(f"♻️ Found local cache {cache_file}. Loading from cache...")
            df_key_metrics = pd.read_parquet(cache_file)
        else:
            ...
            df_key_metrics = pd.DataFrame(raw_data)
            logger.info(f"📊 Processed a total of {df_key_metrics.shape[0]} key metrics rows.")
            
            # Drop duplicates if any overlaps occurred
            if 'symbol' in df_key_metrics.columns and 'date' in df_key_metrics.columns:
                df_key_metrics = df_key_metrics.drop_duplicates(subset=['symbol', 'date'])
            ...
            for str_col in df_key_metrics.select_dtypes(include=['object']).columns:
                df_key_metrics[str_col] = df_key_metrics[str_col].astype(str)
                
            df_key_metrics.to_parquet(cache_file, index=False)
        
        # 4. Save to Bronze
        save_ratios_to_lake(spark, df_key_metrics)
