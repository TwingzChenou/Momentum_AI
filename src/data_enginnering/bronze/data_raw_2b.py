...
def process_and_resample(df_daily, all_tickers):
    """
    Resamples daily data to weekly (W-FRI) and enforces a continuous master date index.
    Keeps rows with missing values (Nulls) for all tickers.
    """
    logger.info("🔧 Processing and resampling data to W-FRI...")
    ...
    # First, setup Date as index for groupby resampling
    ...
    # To keep lines with missing values for everyone, create a master index
    if not resampled.empty:
        min_date = resampled['Date'].min()
        max_date = resampled['Date'].max()
    else:
        # Fallback if no data at all
        max_date = pd.Timestamp.today()
        min_date = max_date - pd.DateOffset(years=2)
        
    # Generate weekly dates ending on Friday
    master_dates = pd.date_range(start=min_date, end=max_date, freq='W-FRI')
    
    # Create the master MultiIndex
    master_df = pd.MultiIndex.from_product([all_tickers, master_dates], names=['Ticker', 'Date']).to_frame(index=False)
    
    # Left join to preserve all master rows
    final_df = pd.merge(master_df, resampled, on=['Ticker', 'Date'], how='left')
    
    logger.info(f"✅ Final shape after resampling and aligning: {final_df.shape}")
    return final_df
...
def save_to_lake(spark, pandas_df):
    ...
    """
    logger.info(f"💾 Saving {pandas_df.shape[0]} rows to {Paths.DATA_RAW_2B}...")
    ...
    try:
        ...
        sdf.write.format("delta") \
            ...
            .save(Paths.DATA_RAW_2B)
            
        logger.info("✅ Success! Raw 2B data saved to Delta Lake.")
...
def main():
    ...
    try:
        ...
        df_weekly = process_and_resample(df_daily, tickers)
        
        # 4. Save to Delta
        save_to_lake(spark, df_weekly)
