...
async def fetch_balance_sheet_for_ticker(session, semaphore, ticker):
    """
    Fetches the quarterly balance sheets for a single ticker.
    Limits concurrency using a semaphore and implements exponential backoff for 429 Rate Limits.
    """
    url = f"{BASE_URL}/financial-growth?symbol={ticker}&limit=1000&period=quarterly&apikey={FMP_API_KEY}"
...
async def fetch_all_balance_sheets(tickers):
    """
    Manages the asynchronous fetching of balance sheets for the entire list of tickers.
    """
    ...
    async with aiohttp.ClientSession() as session:
        for ticker in tickers:
            task = fetch_balance_sheet_for_ticker(session, semaphore, ticker)
            tasks.append(task)
...
def load_unique_tickers_from_history(spark):
    ...
    try:
        ...
        # We only need unique tickers to fetch balance sheets
...
def save_balance_sheets_to_lake(spark, pandas_df):
    """
    Saves the fetched balance sheets DataFrame to Delta Lake.
    """
    logger.info(f"💾 Saving {pandas_df.shape[0]} balance sheet records to {Paths.SP500_FINANCIAL_STATEMENT_GROWTH}...")
    ...
    try:
        ...
        sdf.write.format("delta") \
            ...
            .save(Paths.SP500_FINANCIAL_STATEMENT_GROWTH)
            
        logger.success("✅ Success! Balance Sheets saved.")
        
    except Exception as e:
        logger.error(f"❌ Error saving balance sheets to Lake: {e}")

def main():
    setup_logging()
    logger.info("🚀 Starting Job: List SP500 Balance Sheets Ingestion")

    spark = None

    try:
        spark = create_spark_session(app_name="List_SP500_Balance_Sheets")
        
        cache_file = 'local_balance_sheet_cache.parquet'
        if os.path.exists(cache_file):
            logger.info(f"♻️ Found local cache {cache_file}. Loading from cache...")
            df_bs = pd.read_parquet(cache_file)
        else:
            ...
            # 2. Asynchronously fetch balance sheets from FMP API
            logger.info("⏱️ Fetching balance sheets... This may take a few minutes.")
            raw_data = asyncio.run(fetch_all_balance_sheets(tickers))
            ...
            df_bs = pd.DataFrame(raw_data)
            logger.info(f"📊 Processed a total of {df_bs.shape[0]} balance sheet rows.")
            
            # Drop duplicates if any overlaps occurred
            if 'symbol' in df_bs.columns and 'date' in df_bs.columns:
                df_bs = df_bs.drop_duplicates(subset=['symbol', 'date'])
            ...
            for str_col in df_bs.select_dtypes(include=['object']).columns:
                df_bs[str_col] = df_bs[str_col].astype(str)
                
            df_bs.to_parquet(cache_file, index=False)
        
        # 4. Save to Bronze
        save_balance_sheets_to_lake(spark, df_bs)
