...
def list_sp500_history():
    ...
    """
    logger.info("📡 Connecting to FMP API to fetch S&P 500 history...")
    ...
    try:
        ...
        if df.empty:
            ...
            logger.info("✅ S&P 500 history fetched successfully.")
        return df

    except Exception as e:
        logger.error(f"❌ Error fetching S&P 500 history: {e}")
        return None
...
def save_history_to_lake(spark, pandas_df):
    ...
    """
    logger.info(f"💾 Saving History to {Paths.SP500_LIST_TICKERS} with {pandas_df.shape[0]} rows...")
...
def main():
    ...
    logger.info("🚀 Starting Job: List SP500 History Ingestion")
    ...
    try:
        ...
        spark = create_spark_session(app_name="List_SP500_History_Ingestion")
